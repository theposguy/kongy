...
const GRAVITY           = 600;   // px/sec²
const JUMP_FORCE        = -1000; // px/sec
...
function update(dt){
  ...
  if(!kongy.jumping && jumpPressed && Date.now()-lastJumpTime<150){
    kongy.vy=JUMP_FORCE; kongy.jumping=true; jumpPressed=false; jumpHoldTime=0;
  }
  ...
  if(timeSinceLastC>1.0 && (!lastC || canvas.width-lastC.x > CANDLE_MIN_GAP + Math.random()*100)){
  ...
  powerUps.forEach(p=>{
    if(p.active &&
      ...
      powerUpActive=true; powerUpTimer=10; p.active=false;
    }
  });
  if(powerUpActive){
    powerUpTimer-=dt;
